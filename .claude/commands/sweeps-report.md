...
`.claude/sweep-*-state.csv` files and prints a health dashboard: which
...
Glob `.claude/sweep-*-state.csv`. For each, the sweep name is the middle
...
its command file `.claude/commands/sweep-{name}.md` exists.

Also enumerate the full module list per the discovery rules in
`.claude/commands/_sweep-common.md`, so modules never inspected by any
