...
**Read `.codex/commands/_sweep-common.md` first.** It defines the standard
...
Read `.codex/sweep-dependencies-state.csv` per the state-CSV contract in
...
4. After finishing, update .codex/sweep-dependencies-state.csv following
   the state-CSV contract in .codex/commands/_sweep-common.md, keying
   rows by `unit`. Header:
