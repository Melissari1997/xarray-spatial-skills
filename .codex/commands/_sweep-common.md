...
Each sweep owns one state file: `.codex/sweep-{name}-state.csv`, one row
...
path = Path(".codex/sweep-{name}-state.csv")
...
sweep with `column -t -s, .codex/sweep-{name}-state.csv | less`.
...
```
Ground rules (from .codex/commands/_sweep-common.md):
