...
- If `$REVIEW_CUR_BRANCH` equals `$REVIEW_HEAD_BRANCH` AND
  `$REVIEW_CUR_TOP` contains the segment `.codex/worktrees/`,
...
- Otherwise, create a dedicated review worktree:
  ...
  1. From any path, resolve the main checkout (use `--git-common-dir`
     ...
     git -C "$REVIEW_MAIN" worktree add \
       ".codex/worktrees/pr-$REVIEW_PR_NUM-review" "pr-$REVIEW_PR_NUM-review"
     REVIEW_WT="$REVIEW_MAIN/.codex/worktrees/pr-$REVIEW_PR_NUM-review"
     REVIEW_WT_CREATED=1
     ```
...
5. The review is read-only -- do NOT make commits in this worktree.
   ...
   if [ "${REVIEW_WT_CREATED:-0}" = "1" ]; then
     cd "$REVIEW_MAIN"
     git worktree remove ".codex/worktrees/pr-$REVIEW_PR_NUM-review"
...
REVIEW_BODY_FILE="$REVIEW_BODY_ROOT/.codex/review-pr-$REVIEW_PR_NUM-body.md"
```
