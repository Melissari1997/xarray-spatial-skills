...
1. From the main checkout, create a new branch and worktree using the
   issue number:
   ```bash
   git worktree add .codex/worktrees/issue-<NUMBER> -b issue-<NUMBER>
   ```
...
2. Capture the worktree path and verify isolation before doing
   ...
   ```bash
   ROCKOUT_WT="$(git -C .codex/worktrees/issue-<NUMBER> rev-parse --show-toplevel)"
   ...
   - `$ROCKOUT_WT` ends in `.codex/worktrees/issue-<NUMBER>`.
...
2. Invoke the `/review-pr` command against the PR number from Step 8,
   ...
   `.codex/review-pr-<PR_NUMBER>-body.md` (resolved from the main
...
5. HARD GATE -- independently verify the review is on GitHub before
   ...
   - If not, recover instead of proceeding:
     a. If `.codex/review-pr-<PR_NUMBER>-body.md` exists, post it
