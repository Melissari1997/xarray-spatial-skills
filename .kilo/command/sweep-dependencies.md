...
**Read `.kilo/command/_sweep-common.md` first.** It defines the standard
...
Read `.kilo/worktrees/sweep-dependencies-state.csv` per the state-CSV contract in
...
4. After finishing, update .kilo/worktrees/sweep-dependencies-state.csv following
   the state-CSV contract in .kilo/command/_sweep-common.md, keying
   rows by `unit`. Header:
