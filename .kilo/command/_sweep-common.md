...
Each sweep owns one state file: `.kilo/worktrees/sweep-{name}-state.csv`, one row
...
- The file is tracked in git and registered with `merge=union` in
  ...
  read-update-write cycle below keys rows by `module` and last-write-wins,
...
path = Path(".kilo/worktrees/sweep-{name}-state.csv")
...
sweep with `column -t -s, .kilo/worktrees/sweep-{name}-state.csv | less`.
...
```
Ground rules (from .kilo/command/_sweep-common.md):
