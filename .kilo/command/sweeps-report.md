...
`.kilo/worktrees/sweep-*-state.csv` files and prints a health dashboard: which
...
Glob `.kilo/worktrees/sweep-*-state.csv`. For each, the sweep name is the middle
...
its command file `.kilo/command/sweep-{name}.md` exists.

Also enumerate the full module list per the discovery rules in
`.kilo/command/_sweep-common.md`, so modules never inspected by any
